...
#
# IsaacLab-compatible Planning environment for AirGym.
...
class PlanningIsaacLab(DirectRLEnv):
    """IsaacLab-compatible Planning environment for AirGym.

    This environment implements path planning with:
    - Goal-reaching behavior
    - Obstacle avoidance (implicit through reward)
    """

    cfg: PlanningIsaacLabCfg

    def __init__(self, cfg: PlanningIsaacLabCfg, render_mode: str | None = None, **kwargs):
    ...
    def _reset_idx(self, env_ids):
        ...
        default_root_pose[:, 2:3] = torch.rand(len(env_ids), 1, device=self.device) + 0.5
